export default async function handler(request, response) {
  ...
  const { productName, imageUrl, apiKey, isTest } = request.body;

  if (!apiKey) {
    return response.status(400).json({ error: 'API Key Missing' });
  }

  // 接続テスト用: トークン消費を抑えるための最小プロンプト
  ...
  try {
    ...
    // テストモードなら即OKを返す
    if (isTest) {
        return response.status(200).json({ status: 'OK', message: 'Connection Successful' });
    }
